...
contract AxoloRegistry {
    ...
    // Apenas endereços com REGISTRAR_ROLE podem chamar funções de escrita
    modifier onlyRegistrar() {
        require(
            accessControl.hasRole(REGISTRAR_ROLE, msg.sender),
            "AxoloRegistry: caller is not a registrar"
        );
        _;
    }
    ...
    ) external onlyRegistrar returns (uint256 tankId) {
    ...
    function deactivateTank(uint256 tankId) external onlyRegistrar {
        require(tanks[tankId].id != 0, "AxoloRegistry: tank does not exist");
    ...
    function registerAxolotl(
        ...
        bytes32 photoHash
    ) external onlyRegistrar returns (uint256 axolotlId) {
        require(tanks[tankId].id != 0, "AxoloRegistry: tank does not exist");
        require(tanks[tankId].active, "AxoloRegistry: tank is not active");
        require(bytes(name).length > 0, "AxoloRegistry: name cannot be empty");
    ...
    function transferAxolotl(uint256 axolotlId, uint256 newTankId) external onlyRegistrar {
        require(axolotls[axolotlId].id != 0, "AxoloRegistry: axolotl does not exist");
        require(axolotls[axolotlId].active, "AxoloRegistry: axolotl is not active");
    ...
    function deactivateAxolotl(uint256 axolotlId) external onlyRegistrar {
        require(axolotls[axolotlId].id != 0, "AxoloRegistry: axolotl does not exist");
